...
def ellipsoid(ellipsoid_matrix, center, delta, oracle):
    ...
    history += [(answer, function_value, gradient_value, ellipsoid_matrix)]
...
def inside(ellipsoid_matrix, delta):
    ...
    eigenvalues = np.sqrt(np.linalg.eigvals(ellipsoid_matrix))
...
print('answer: {0}, best_point: {1}, best_value: {2}, best_gradient: {3}'.format(answer, best_point, best_value, best_gradient))
