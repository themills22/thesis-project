...
systems, solution_counts = rjd.read_file(args.matrix_file, args.matrix_size)
system_indices = list(range(systems.shape[0]))
results = np.zeros((len(system_indices), 4))
for i in tqdm.tqdm(system_indices):
    ...
    results[i] = np.array([solution_counts[i], approximation, scale_time_taken, approximation_time_taken])
